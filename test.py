...
if __name__ == "__main__":
    mqtt_client = MqttClient()
    mqtt_client.connect("127.0.0.1", 1883)
    send_message_sensor(1, mqtt_client, True, 3)
    send_message_sensor(2, mqtt_client, True, 3)
    send_message_sensor(3, mqtt_client, True, 3)
    send_message_sensor(4, mqtt_client, True, 3)
    send_message_sensor(5, mqtt_client, True, 3)
    send_message_sensor(5, mqtt_client, True, 3)
    send_message_sensor(4, mqtt_client, True, 3)
    send_message_sensor(3, mqtt_client, True, 3)
    send_message_sensor(2, mqtt_client, True, 3)
    send_message_sensor(1, mqtt_client, True, 3)
    send_message_sensor(1, mqtt_client, False, 3)
    #